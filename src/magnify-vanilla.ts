...
class Magnify {
    ...
    constructor({ originImage, zoomImage = undefined, options = undefined }: Pick<Magnify, 'originImage' | 'zoomImage' | 'options'>) {
        ...
        this.nativeImg.src = this.zoomImage ? this.zoomImage.src : this.originImage.src;
    ...
    getComputed(element: HTMLElement) {
    ...
    onMoveTouch(event: TouchEvent) {
        event.preventDefault();
        this.onMove(event.touches[0].pageX, event.touches[0].pageY);
    }

    onMoveDefault(event: MouseEvent | PointerEvent) {
        event.preventDefault();
    ...
    onMove(pageX: number, pageY: number) {
        ...
        const relX = pageX - elementOffsetLeft;
        const relY = pageY - elementOffsetTop;
        ...
        const backgroundSize = nativeImgWidth * optionScale + 'px ' + nativeImgHeight * optionScale + 'px';
